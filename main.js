...
class HeroScene {
  ...
  init() {
    ...
    const pointLight1 = new THREE.PointLight(0xff00e4, 2, 20);
    pointLight1.position.set(3, 3, 3);
    this.scene.add(pointLight1);

    const pointLight2 = new THREE.PointLight(0x33ccff, 2, 20);
  ...
  createRayBursts() {
    // Colorful ray lines radiating outward (inspired by Josh's character rays)
...
function initHeroAnimation() {
  ...
  timeline
    ...
    .from('.hero-line', { opacity: 0, y: 40, duration: 0.8, stagger: 0.15 }, '-=0.4')
    .from('.hero-subtitle', { opacity: 0, y: 20, duration: 0.7 }, '-=0.4')
    .from('.hero-cta .btn-hero-primary', { opacity: 0, y: 20, duration: 0.6 }, '-=0.3')
    .from('.hero-cta .btn-hero-secondary', { opacity: 0, y: 20, duration: 0.6 }, '-=0.4')
